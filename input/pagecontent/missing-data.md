...
**Rule**: If the source system does not have data for an element with a minimum cardinality = 0 (including elements labeled [Must Support](must-support.html)), the data element **SHALL** be omitted from the resource.
...
2. **If the ValueSet does not have an appropriate "unknown" concept code**, you **MUST** use a concept from the ValueSet. Otherwise, the instance will not be conformant.
