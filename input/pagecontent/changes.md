...
- **Added:** Profile [Patient Pseudonymized](StructureDefinition-mii-pr-person-patient-pseudonymisiert.html): Added optional `type.coding` sub-slice `mr` (`http://terminology.hl7.org/CodeSystem/v2-0203|MR`) to `PseudonymisierterIdentifier`. When the pseudonym replaces a Medical Record Number, this code SHOULD be set to allow site-independent discovery of the pseudonymized identifier. See: [#63](https://github.com/medizininformatik-initiative/kerndatensatz-basis/issues/63)
...
- **Added**: Profile [Patient Pseudonymized](StructureDefinition-mii-pr-person-patient-pseudonymisiert.html) now contains an optional identifier for masked health insurance ID under `Patient.identifier`
...
- **Changed**: Cardinality of `Encounter.period` in the [Encounter](StructureDefinition-mii-pr-fall-kontakt-gesundheitseinrichtung.html) profile from 1..1 to 0..1 to represent planned encounters where the actual time period is still unknown. See: [#56](https://github.com/medizininformatik-initiative/kerndatensatzmodul-fall/issues/56)
...
- **Added**: New profile [Cause of Death](StructureDefinition-mii-pr-person-todesursache.html) for representing cause of death using ICD-10-WHO
...
- **Added**: In the [Diagnosis](StructureDefinition-mii-pr-diagnose-condition.html) profile, elements `Condition.code.coding` and `Condition.bodySite.coding` were each extended with an OPTIONAL slice for ICD-O-3
