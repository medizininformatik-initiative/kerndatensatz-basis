...
- **Added:** Das neue [MII ValueSet Person Recorded Sex or Gender SNOMED](ValueSet-mii-vs-person-recordedsexorgender-snomed.html) ist mit der Bindungsstärke `preferred` an `Patient.extension:recordedSexOrGender.extension:value` gebunden. Es enthält SNOMED-CT-Befunde zum biologischen Geschlecht und den Qualifier-Wert `Unknown`.
...
- **Changed**: Im [Patient](StructureDefinition-mii-pr-person-patient.html)-Profil wurden alle min 1.. Kardinalitäten entfernt
- **Added**: Profil [Patient Pseudonymisiert](StructureDefinition-mii-pr-person-patient-pseudonymisiert.html) ist neu seit Version 1.0. Im Vergleich zu zuvor veröffentlichten Versionen wurden alle 0..0 Kardinalitäten entfernt. Außerdem werden vom Profil keine Pseudonymisierungsregeln mehr vorgeschrieben. Die genauen Pseudonymisierungsregeln innerhalb der Medizininformatik-Initiative werden noch entwickelt und können in einer späteren Version des Profils angewendet werden. Folglich wurden die folgenden Invarianten entfernt:
...
- **Added**: Neues Profil [Todesursache](StructureDefinition-mii-pr-person-todesursache.html) zur Darstellung der Todesursache mit ICD-10-WHO
...
- **Added**: Im [Diagnose](StructureDefinition-mii-pr-diagnose-condition.html)-Profil wurden die Elemente `Condition.code.coding` und `Condition.bodySite.coding` jeweils mit einem OPTIONALEN Slice für ICD-O-3 erweitert
