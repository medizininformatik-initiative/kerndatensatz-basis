# Handling Missing Data - MII Implementation Guide Core Dataset Base v2027.0.0-ballot.rc1
...
**Rule**: §missing-data-1:If the source system does not have data for an element with a minimum cardinality = 0 (including elements labeled [Must Support](must-support.md)), the data element **SHALL** be omitted from the resource.§
...
1. §missing-data-5:For elements with a**required**binding, if the**ValueSet**does not have an appropriate "unknown" concept code, you**SHALL**use a concept from the**ValueSet**(otherwise the instance will not be conformant).§
