...
* **Added:** The new [MII ValueSet Person Recorded Sex or Gender SNOMED](ValueSet-mii-vs-person-recordedsexorgender-snomed.md) is bound with `preferred` strength to `Patient.extension:recordedSexOrGender.extension:value`. It includes SNOMED CT findings related to biological sex and the `Unknown` qualifier value.
